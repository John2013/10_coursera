...
def get_random_courses_list(courses_list_xml, courses_amount):
    xml_root = etree.XML(courses_list_xml)
...
if __name__ == '__main__':
    ...
    courses_list_xml = get_web_content(
        'https://www.coursera.org/sitemap~www~courses.xml'
    )

    courses_links = get_random_courses_list(courses_list_xml, courses_count)
    ...
    for course_link in courses_links:
        course_html = get_web_content(course_link)
        courses_list.append(get_course_info(course_html))
